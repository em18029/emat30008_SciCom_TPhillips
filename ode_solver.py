...
def euler_step(x,y,deltat_max):
    return y + deltat_max * f


def solve_to(f, x0,t,deltat_max):
    y=np.zeros(t)
    y[0]=x0
    for i in range(1,len(t)):
        y[i+1] = euler_step(f(y[i]), y[i], deltat_max)
    return y


def solve_ode(solve_in,deltat_max):
    generate numerical soluition for solving

    call solve to function which calls euler step to generate solution estiamtes     
    ...
    t = np.linspace(a,b,(a-b)*deltat_max)
    x_solution = solve_to(f_tosolve,x0,t,deltat_max)


    return t, x_solution


t,x_sols=solve_ode([0,1],0.01)
